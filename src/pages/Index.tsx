...
const Index = () => {
  ...
  const { gameState, stats, makeGuess, resetGame, shareResult, maxGuesses, canPlayMore, gamesRemaining } =
    useGameState();

  const correctGuessedCountries = gameState.guesses.filter((g) => g.isOnPath).map((g) => g.country);
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center px-4 py-6">
          ...
          {!gameState.gameOver && gameState.guesses.length > 0 && (
            <p className="mt-4 text-sm text-muted-foreground">Find all countries on the path to win!</p>
          )}
        </main>
